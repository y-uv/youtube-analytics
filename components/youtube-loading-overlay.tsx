...
export function YouTubeLoadingOverlay() {
  ...
  useEffect(() => {
    ...
    // Simulate progress increasing automatically to 99% over 20 seconds
  useEffect(() => {
    ...
    // Calculate how much to increment every interval to reach 99% in 20 seconds
    // 20 seconds = 20000ms
    // If we update every 200ms, that's 100 updates to reach 99%
    // So each update should increase by approximately 0.99 (99/100)
    const interval = setInterval(() => {
      setLoadingProgress(prev => {
        // Move to 99% over 20 seconds in a linear fashion
        if (prev < 99) {
          return prev + 0.99; // Move approximately 1% each interval
        }
        return 99; // Stay at 99%
      });
    }, 200); // 100 updates over 20 seconds = update every 200ms
    
    return () => clearInterval(interval);
  }, [isClient]);
    return (
    <div className="fixed inset-0 z-50 flex items-center justify-center pt-20 overflow-hidden">
      ...
      <div className="w-full max-w-3xl bg-white dark:bg-zinc-900 rounded-lg overflow-hidden shadow-2xl flex flex-col" style={{ maxHeight: "600px" }}>
        ...
        <div className="bg-gray-100 dark:bg-black flex items-center justify-center relative overflow-hidden" style={{ height: "280px" }}>
          {/* Loading GIF in the center of the player - optimized to fit properly */}          <img
            src={loadingGif}
          ...
          <div className="absolute top-0 left-0 right-0 h-1.5 bg-gray-200 dark:bg-zinc-700">
            <div 
              className="h-full bg-red-600 transition-all duration-500 ease-linear shadow-sm"
              style={{ width: `${loadingProgress}%` }}
            ></div>
          </div>
            {/* Loading progress indicator has been removed as requested */}
        </div>
        {/* Video information section */}
        <div className="p-4 overflow-hidden" style={{ maxHeight: "none" }}>
          {/* Video title */}
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{videoTitle}</h2>
          
          {/* Video stats */}
          <div className="flex items-center text-gray-600 dark:text-zinc-400 text-sm mb-4">
          ...
          {/* Engagement buttons */}
          <div className="flex items-center justify-between border-b border-t border-gray-200 dark:border-zinc-800 py-2 mb-4">
            <div className="flex items-center gap-4">              <button className="flex items-center gap-1 text-gray-700 dark:text-zinc-300">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M18.77,11h-4.23l1.52-4.94C16.38,5.03,15.54,4,14.38,4c-0.58,0-1.14,0.24-1.52,0.65L7,11H3v10h4h1h9.43 c1.06,0,1.98-0.67,2.19-1.61l1.34-6C21.23,12.15,20.18,11,18.77,11z M7,20H4v-8h3V20z M19.98,13.17l-1.34,6 C18.54,19.65,18.03,20,17.43,20H8v-8.61l5.6-6.06C13.79,5.12,14.08,5,14.38,5c0.26,0,0.5,0.11,0.63,0.3 c0.07,0.1,0.15,0.26,0.09,0.47l-1.52,4.94L13.18,12h1.35h4.23c0.41,0,0.8,0.17,1.03,0.46C19.92,12.61,20.05,12.86,19.98,13.17z"></path>
                </svg>
                <span>{formatNumber(likeCount)}</span>
              </button>
              <button className="flex items-center gap-1 text-gray-700 dark:text-zinc-300">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-gray-900 dark:text-white font-medium">{channelName}</h3>
                <p className="text-gray-600 dark:text-zinc-400 text-sm">{formatNumber(subscriberCount)} subscribers</p>
              </div>
            </div>
          ...
          {/* Processing message card removed as requested */}
        </div>
      </div>
    </div>
  )
